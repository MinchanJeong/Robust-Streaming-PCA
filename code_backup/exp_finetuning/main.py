...
# Finetuning
# B_list for npm50_k2_noisy
# B_list = np.arange(100,600,5)
# B_list for npm100_k2_noisy
# B_list = np.arange(350,570,5)
# B_list for npm125_k2_noisy
# B_list = np.arange(350,650,5)
# B_list for npm150_k2_noisy
# B_list = np.arange(400,650,5)
# B_list for npm200_k2_noisy
# B_list = np.arange(400,1000,10)
...
parser.add_argument('-loop', type=int, nargs='?', default = 1, help='loop(int)')
...
parser.add_argument('-delta', type=float, nargs='?', default = 1.6, help='delta(float)')
...
parser.add_argument('-label', type=str, nargs='?', default = 'defaultlabel', help='savelabel')
...
savepath = './data/{}/{}'.format(workspace,savelabel)
...
hyperdict = {'T':T, 'T_save':T_save, 'loop':loop, 'k':k, 'p':p, 'sigma':sigma,\
            ...
            'algo':algo, 'lp_list':learning_param_list, 'label':savelabel, 'rotate_type':rotate_type}
